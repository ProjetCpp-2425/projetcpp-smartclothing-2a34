...
Employe::Employe(const QString &Nom, const QString &Prenom, const QString &Email, int Telephone,
    ...
    Date_embauche(Date_embauche),description(description),motedepass(motedepass),fingerprintData(fingerprint){}
Employe::Employe()
...
bool Employe::save() {
    ...
    // Prepare the SQL query to insert the fingerprint data into the database
    QSqlQuery query;
    query.prepare("INSERT INTO Employe (id_employe, fingerprint_data) VALUES (:id_employe, :fingerprint_data)");

    // Bind the values
    query.bindValue(":id_employe", Id_employe);
    query.bindValue(":fingerprint_data", fingerprintData);

    // Execute the query and check for success
    if (!query.exec()) {
        qDebug() << "Error saving fingerprint: " << query.lastError().text();
        return false;
    }
...
bool Employe::loadfs() {
    QSqlQuery query;
    query.prepare("SELECT * FROM EMPLOYE WHERE fingerprint = :fingerprint");
    query.bindValue(":fingerprint", fingerprintData);
    if (query.exec() && query.next()) {
        Id_employe = query.value("Id_employe").toString();
        Nom = query.value("Nom").toString();
        Prenom = query.value("Prenom").toString();
        Email = query.value("Email").toString();
        Telephone = query.value("Telephone").toInt();
